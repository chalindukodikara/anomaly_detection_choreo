...
#Plot original data
train.plot(xlabel = 'Year', ylabel = 'Sales', title = 'Original Series')
print(train)
plt.show()
...
plot_acf(train)
plot_pacf(train, lags = 11)
plt.show()

#Take first difference
train['1difference']=train['Sales']-train['Sales'].shift(1)
...
plot_pacf(train['1difference'].dropna(), title = 'PACF Diagram after 1st difference', lags = 11)
plt.show()
...
# model = ARIMA(train['Sales'], order=(11,1,1))
model=SARIMAX(train['Sales'], order=(7,1,1), seasonal_order=(1, 0, 0, 10))
result = model.fit()
...
# df2['predictions']=result.predict(start=24,end=35)
# print(df2)
# df2[['Sales','predictions']].plot(title = 'Predictions for test test values')
# plt.show()
#
#
# new_dates=[df.index[-1]+DateOffset(months=x) for x in range(1,20)]
# df_pred=pd.DataFrame(index=new_dates,columns =df.columns)
# df_pred.head()
#
# df3=pd.concat([df2, df_pred])
# df3['predictions']=result.predict(start=24,end=55)
# df3[['Sales','predictions']].plot(title = 'Predictions for future values not in dataset')
# plt.show()
# #
# # print(df3)
# #
...
# test['predictions'] = np.nan
df2 = pd.concat([train, test])
prediction = result.predict(start=25,end=35)
...
df2[['Sales','predictions']].plot(title = 'Predictions for test set values')
plt.plot(testAnomaly.index, testAnomaly['Sales'], color = 'red', label = 'Anomaly')
# plt.legend()
plt.show()
