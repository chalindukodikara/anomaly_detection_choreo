...
###################### ARIMA MODEL ###################################
######################################################################
def main(train, test, dataSetID, column, train_v1, test_v1):
    ...
    testSetIsAnomaly, models = trainModel(train, test, numOfSteps, immediate1stPredictions, immediate2ndPredictions, immediate3rdPredictions, dataSetID, column, test_v1)
...
def trainModel(train, test, numOfSteps, immediate1stPredictions, immediate2ndPredictions, immediate3rdPredictions, dataSetID, column, test_v1):
    ...
    plt.legend(loc='best')
    plt.savefig("plots_result/" + str(dataSetID) + '. ' + 'Train data and fitted model' + '.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plotResults(dataSet, trainSet, testSet, model, title1, title2, splitLineValue, dataSetID):
    ...
    plt.legend(loc='best')
    plt.savefig("plots_result/" + str(dataSetID) + '. ' + title1 + '.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # plt.plot(trainSet.index, model.fittedvalues, label='Fitted model')
    plt.legend(loc='best')
    plt.savefig("plots_result/" + str(dataSetID) + '. ' + title2 + '.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # plt.plot(trainSet.index, model.fittedvalues, label='Fitted model')
    plt.legend(loc='best')
    plt.savefig("plots_result/" + str(dataSetID) + '. ' + title2 + ' (Much smaller).png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plotAnomaly(test, dataSet, train, dataSetID, test_v1, column):
    ...
    plt.legend(loc='best')
    plt.savefig("plots_result/" + str(dataSetID) + '.' + ' Anomaly whole test set 1.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.scatter(testAnomaly.index, testAnomaly[column], marker='o', color='red', label='Anomaly')
    plt.legend(loc='best')
    plt.savefig("plots_result/" + str(dataSetID) + '.' + ' Anomaly whole test set 2.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    for i in range(int(len(test) / 30)):
        ...
        plt.scatter(testAnomaly.index, testAnomaly[column], marker='o', color='red', label='Anomaly')
        plt.legend(loc='best')
        plt.savefig("plots_result/" + str(dataSetID) + '.' + ' Anomaly Detection- Time period ' + str(i + 1) + '.png', dpi=300,
                    bbox_inches='tight')
        plt.show()

################################################################################################################
################################ Changed frequency of updates ##################################################
################################################################################################################
# import matplotlib.pyplot as plt
# import numpy as np
# import pandas as pd
...
# ######################################################################
# def main(train, test, dataSetID, column, train_v1, test_v1):
#     numOfSteps = 7 # Num of steps that need to forecasted
#     # test = test[:100] # Test set size
#     immediate1stPredictions = []
#     immediate2ndPredictions = []
#     immediate3rdPredictions = []
#     dataSet = pd.concat([train, test], ignore_index=False, axis=0)
...
#     plt.plot(train.index, result.predict(1, len(train)), label='Fitted model')
#     plt.legend(loc='best')
#     plt.savefig('plots/' + str(dataSetID) + '. ' + 'Train data and fitted model' + '.png', dpi=300, bbox_inches='tight')
#     plt.show()
#
#     # Plot train data and fitted model in small time frame
#     # train.plot(figsize=(10, 7), xlabel='Time', ylabel='Latency', label='Latency',
...
#
#         squared_errors = abs(result.resid.values)
#         threshold = np.mean(squared_errors) + 3 * np.std(squared_errors) + np.percentile(squared_errors, [94])[0]
#         # threshold = np.percentile(squared_errors, [94])[0]
...
#
# def detectAnomaly(predictedValue, i, test, threshold, test_v1):
#     squaredError = abs(predictedValue - test.iloc[i, 0])
#     squaredError_1 = (predictedValue - test_v1.iloc[i, 0]) ** 2
...
#     plt.axvline(pd.to_datetime(splitLineValue), color='k', linestyle='--')
#     plt.legend(loc='best')
#     plt.savefig('plots/' + str(dataSetID) + '. ' + title1 + '.png', dpi=300, bbox_inches='tight')
#     plt.show()
#
#     # Plot whole dataset, fitted values and immediate observations in small time frame, index -11
#     dataSet.plot(figsize=(10, 7), xlabel='Time', ylabel='Latency', label='Latency',
...
#     # plt.plot(trainSet.index, model.fittedvalues, label='Fitted model')
#     plt.legend(loc='best')
#     plt.savefig('plots/' + str(dataSetID) + '. ' + title2 + '.png', dpi=300, bbox_inches='tight')
#     plt.show()
#
#     #
#     # Plot whole dataset, fitted values and immediate observations in small time frame, -9
...
#     # plt.plot(trainSet.index, model.fittedvalues, label='Fitted model')
#     plt.legend(loc='best')
#     plt.savefig('plots/' + str(dataSetID) + '. ' + title2 + ' (Much smaller).png', dpi=300, bbox_inches='tight')
#     plt.show()
#
# def plotAnomaly(test, dataSet, train, dataSetID, test_v1, column):
...
#
#     if columnValues.max() < 2:
#         maxValue = round(columnValues.max() + 0.2 * abs(dataSet.iloc[0, 0] - dataSet.iloc[1, 0]), 3)
#         minValue = round(columnValues.min() - 0.2 * abs(dataSet.iloc[0, 0] - dataSet.iloc[1, 0]), 3)
#     else:
#         maxValue = round(columnValues.max() + 1.5 * abs(dataSet.iloc[0, 0] - dataSet.iloc[1, 0]), 3)
#         minValue = round(columnValues.min() - 1.5 * abs(dataSet.iloc[0, 0] - dataSet.iloc[1, 0]), 3)
#
#     # Increase some amount of Y limit
#
#     # maxValue = round(columnValues.max() + 1.5 * constantHigh, 3)
...
#     plt.scatter(testAnomaly.index, testAnomaly[column], marker='o', color='red', label='Anomaly')
#     plt.legend(loc='best')
#     plt.savefig('plots/' + str(dataSetID) + '.' + ' Anomaly whole test set 1.png', dpi=300, bbox_inches='tight')
#     plt.show()
#
#     printSet = dataSet.copy()
#     # printSet = printSet.loc[str(train.index[-13])[:-9]:str(test.index[int((len(test) / 3) * 1)])[:-9]]
...
#     plt.scatter(testAnomaly.index, testAnomaly[column], marker='o', color='red', label='Anomaly')
#     plt.legend(loc='best')
#     plt.savefig('plots/' + str(dataSetID) + '.' + ' Anomaly whole test set 2.png', dpi=300, bbox_inches='tight')
#     plt.show()
#
#     for i in range(int(len(test) / 30)):
#         printSet = dataSet.copy()
...
#         plt.scatter(testAnomaly.index, testAnomaly[column], marker='o', color='red', label='Anomaly')
#         plt.legend(loc='best')
#         plt.savefig('plots/' + str(dataSetID) + '.' + ' Anomaly Detection- Time period ' + str(i + 1) + '.png', dpi=300,
#                     bbox_inches='tight')
#         plt.show()
